...
class HdfilmcehennemiV2 : MainAPI() {
    override var mainUrl = "https://hdfilmcehennemi10.org"
    override var name = "HdfilmcehennemiV2"
    ...
    private suspend fun Element.sonBolumler(): SearchResponse? {
        val name = this.selectFirst("div.episode-title div.serie-name a")?.text() ?: return null
        val episode = this.selectFirst("div.episode-title div.episode-name a")?.text()?.trim()?.replace(". Sezon ", "x")?.replace(". Bölüm", "") ?: return null
        val title = "$name\n$episode"

        val href = fixUrlNull(this.selectFirst("div.episode-title div.serie-name a")?.attr("href")) ?: return null
        val posterUrl = fixUrlNull(this.selectFirst("div.poster div.img a img.lazy")?.attr("data-src"))
